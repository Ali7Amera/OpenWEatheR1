...
#   - Ali Hisham Amera              (24040377)
#   - Ahmed Mohamad Abdelrahman     (24040817)
#   - Ahmed Mohamed Abdelhamid      (24040324)
#   - Ahmed Yasser Helmy            (24040328)
# ============================================================
...
# ------------------------------------------------------------
# API Configuration
# Contributed by: Ali Hesham Amera (24040377)
...
# Contributed by: Ahmed Mohamed Abdelhamid (24040324)
# Fetches current weather data for the city entered by the user.
# Handles HTTP errors (401 Unauthorized, 404 Not Found) and
# falls back to the Geocoding API when city name search fails.
# ------------------------------------------------------------
def get_weather():
    ...
    try:
        # Primary request using city name
        url = (
            f"https://api.openweathermap.org/data/2.5/weather"
            f"?q={city}&appid={API_KEY}&units=metric&lang=ar"
        )
        ...
        if response.status_code == 200:
            ...
            result_label.config(
                fg="#e74c3c",
                text="Invalid API Key or not activated yet"
            )

        elif response.status_code == 404:
            # Fallback: use Geocoding API to resolve city coordinates
            try:
                geo_url = (
                ...
                if geo_res:
                    ...
                    url2 = (
                        f"https://api.openweathermap.org/data/2.5/weather"
                        f"?lat={lat}&lon={lon}&appid={API_KEY}&units=metric&lang=ar"
                    )
            ...
            result_label.config(
                fg="#e74c3c",
                text=f"Error {response.status_code}: {data.get('message', 'Unknown error')}"
            )
...
# ------------------------------------------------------------
# Function: show_weather(data, city)
# Contributed by: Ahmed Mohamed Abdelhamid (24040324)
# Parses the JSON response from the API and displays
...
# Contributed by: Ahmed Mohamed Abdelhamid (24040324)
# Clears the city input field and resets the result display.
# ------------------------------------------------------------
def clear_result():
...
# ── GUI Setup ────────────────────────────────────────────────
# Contributed by: Ahmed Mohamed Abdelhamid (24040324)
# Builds the Tkinter window with entry field, buttons, and
# a result label to display weather information.
# ------------------------------------------------------------
root = tk.Tk()
